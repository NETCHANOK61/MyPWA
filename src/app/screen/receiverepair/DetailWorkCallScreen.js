...
export default function DetailWorkCallScreen(props) {
      ...
      setTimeout(() => {
        ...
        switch (key) {
          case 'ALERT_INSERT_SUCCESS':
            setStateAlert(1, true, false, 'บันทึกข้อมูลเรียบร้อย', 'ตกลง', '', () => {
          ...
          case 'ALERT_INSERT_FAILED':
            setStateAlert(4, true, false, 'ไม่สามารถบันทึกข้อมูลได้', 'ตกลง', '', () => {
              setVisibleAlert(false);
            }, () => { });
            break;
          case 'ALERT_ACCEPT_FAILED':
            setStateAlert(4, true, false, value.message, 'ตกลง', '', () => {
              setVisibleAlert(false);
            }, () => { });
            break;
          case 'ALERT_REJECT_VERIFLY':
            setStateAlert(3, true, false, 'กรุณาเลือกเหตุผลเเละกรอกรายละเอียด', 'ตกลง', '', () => {
              setVisibleAlert(false);
            }, () => { });
            break;
          case 'ALERT_LOCATION':
            setStateAlert(4, true, false, 'ไม่พบตำแหน่งจุดซ่อม', 'ตกลง', '', () => {
              setVisibleAlert(false);
              setSelectedIndex(0);
            }, () => { });
            break;
  ...
  const rejectTypeList = () => {
    if (receiveRepairDetailReducer.dataArrayTypeReject) {
      return receiveRepairDetailReducer.dataArrayTypeReject.map((l, i) => {
        return l.text;
      });
    } else {
      return [];
  ...
  const handlersReject = () => {
    if (pickedData == undefined || remark == '') {
  ...
  const icon_4 = () => (
    <View style={{ alignItems: 'center' }}>
      <MaterialCommunityIcons
        ...
        size={20}
      />
      <Text style={[textsty.text_sm_regular, { color: 'white' }]}>ไม่รับงาน</Text>
    </View>
  );
  const buttons = [
    { element: icon_1 },
      ...
      <View>
        <Overlay
          ...
          <Card containerStyle={detailWorkCallStyle.card}>
            ...
            <Card.Divider />
            <Text style={textsty.text_normal_regular}>
              {'เหตุผล'}<Text style={{ color: 'red' }}>{'*'}</Text>
            </Text>
            <SelectDropdown
              data={rejectTypeList()}
        ...
        <Overlay
          ...
          <Card containerStyle={[detailWorkCallStyle.card, { width: 0.8 * width, alignItems: 'center' }]}>
            <Card.Title style={[textsty.text_normal_bold_color_blue, { alignItems: 'center', alignSelf: 'center' }]}>
              <View >
                <Image
            ...
            <View style={{ flexDirection: 'row' }}>
              <TouchableOpacity
                ...
                <Text style={[textsty.text_normal_regular, { color: 'white' }]}>{'ตกลง'}</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={{
                  flex: 1,
                ...
                <Text style={[textsty.text_normal_regular, { color: 'white' }]}>{'ยกเลิก'}</Text>
              </TouchableOpacity>
            </View>
          </Card>
        </Overlay>
